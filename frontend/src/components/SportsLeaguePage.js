...
function SportsLeaguePage() {
  ...
  return (
    <div className="min-h-screen bg-[#070b14] text-white p-6 pb-24" dir="rtl text-right">
      {/* Header */}
                    ...
                    {standings.map((t, idx) => (
                        <div key={idx} className="flex flex-col items-center gap-2 shrink-0">
                            <div className="w-20 h-20 rounded-full bg-gray-800 border-2 border-emerald-500/30 flex items-center justify-center p-3 shadow-xl">
                                <div className="w-full h-full rounded-full bg-emerald-500/10 flex items-center justify-center text-[10px] font-black text-center leading-tight whitespace-pre-wrap">{t.team}</div>
                            </div>
                            <span className="text-[10px] font-bold text-gray-400">{t.team}</span>
                        </div>
                    ))}
                </div>
            </section>
                    ...
                    {completedMatches.map((m, idx) => (
                        <div key={idx} className="bg-gray-800/40 rounded-3xl p-6 border border-white/5 flex items-center justify-between">
                            <div className="flex-1 text-center">
                                <p className="font-bold mb-1 text-sm">{m.team1}</p>
                            </div>
                            <div className="px-6 flex items-center gap-4">
                                <span className={`text-2xl font-black italic ${m.score1 > m.score2 ? 'text-emerald-500' : 'text-white'}`}>{m.score1}</span>
                                <span className="w-px h-8 bg-white/10"></span>
                                <span className={`text-2xl font-black italic ${m.score2 > m.score1 ? 'text-emerald-500' : 'text-white'}`}>{m.score2}</span>
                            </div>
                            <div className="flex-1 text-center">
                                <p className="font-bold mb-1 text-sm">{m.team2}</p>
                            </div>
                        </div>
                    ))}
                </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-[#070b14]/80 backdrop-blur-xl px-12 py-4 flex justify-between items-center border-t border-white/5 z-50">
        ...
        <div className="w-12 h-12 bg-emerald-600 rounded-2xl flex items-center justify-center -mt-10 shadow-lg shadow-emerald-500/20 text-white">
