    ...
    <div className="min-h-screen bg-[#0a0f1e] text-white p-6 pb-32 font-sans" dir="rtl">
      ...
      <div className="relative rounded-[2.5rem] overflow-hidden mb-10 h-64 shadow-2xl group border border-white/5">
        ...
        <div className="absolute inset-0 z-20 flex flex-col justify-end p-8">
          ...
          <h2 className="text-3xl font-black italic leading-none mb-1">بطولة نجوم غراس 2026</h2>
      ...
      <div className="flex gap-2 bg-[#1a1f2e]/50 p-1.5 rounded-2xl mb-8 border border-white/5">
        {["ranking", "results", "teams"].map((tab) => (
          <button 
            key={tab}
            onClick={() => setActiveTab(tab)} 
            className={`flex-1 py-3 rounded-xl font-black text-xs transition-all ${
                activeTab === tab ? "bg-[#006d44] text-white shadow-xl scale-[1.02]" : "text-gray-500"
            }`}
          >
            {tab === "ranking" ? "الترتيب العام" : tab === "results" ? "آخر النتائج" : "الفرق المشاركة"}
          </button>
        ))}
      </div>
            ...
            {standings.map((team, index) => (
              <div key={index} className="bg-[#151a28] rounded-[1.5rem] p-5 flex items-center justify-between border border-white/5 hover:border-emerald-500/30 transition-all">
                <div className="flex items-center gap-4">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center font-black italic ${
        ...
        {activeTab === "results" && (
          <div className="space-y-4">
            {results.map((match, index) => (
              <div key={index} className="bg-[#151a28] rounded-[2rem] p-6 border border-white/5">
                 <div className="flex items-center justify-between gap-4">
                    <div className="flex-1 text-center">
                        <p className="font-black text-xs mb-2">{match.team1}</p>
                        <p className="text-3xl font-black italic">{match.score1}</p>
                    </div>
                    <div className="text-gray-700 font-black italic text-xl">-</div>
                    <div className="flex-1 text-center">
                        <p className="font-black text-xs mb-2">{match.team2}</p>
                        <p className="text-3xl font-black italic">{match.score2}</p>
                    </div>
                 </div>
                 <div className="text-center mt-4">
                    <span className="text-[8px] font-black bg-white/5 px-3 py-1 rounded-full text-gray-500">نهاية المباراة</span>
                 </div>
              </div>
            ))}
          </div>
        ...
        {activeTab === "teams" && (
           <div className="grid grid-cols-2 gap-4">
              {teams.map((team, index) => (
                <div key={index} className="bg-[#151a28] rounded-[1.5rem] p-4 text-center border border-white/5">
                    {team.group_photo ? (
                        <img src={team.group_photo} alt={team.name} className="w-full h-24 object-cover rounded-xl mb-3 shadow-lg" />
                    ) : (
                        <div className="w-full h-24 bg-[#1f2637] rounded-xl mb-3 flex items-center justify-center text-2xl">🛡️</div>
                    )}
                    <h3 className="font-black text-xs">{team.name}</h3>
                </div>
              ))}
           </div>
        )}
      ...
      <div className="fixed bottom-6 left-6 right-6 z-50">
        <div className="bg-[#151a28]/80 backdrop-blur-2xl px-6 py-4 rounded-[2rem] border border-white/10 shadow-2xl flex justify-between items-center max-w-lg mx-auto">
          {navItems.map((item, idx) => (
            <Link key={idx} to={item.path} className="flex flex-col items-center gap-1 group">
              <span className={`text-xl transition-transform group-active:scale-95 ${item.active ? "opacity-100" : "opacity-40"}`}>{item.icon}</span>
              <span className={`text-[8px] font-black tracking-tighter ${item.active ? "text-emerald-500" : "text-gray-500"}`}>{item.label}</span>
            </Link>
          ))}
        </div>
      </div>
