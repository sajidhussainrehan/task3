    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-2">
        <button
          ...
          className={`flex-1 py-3 rounded-xl font-bold transition-all border-2 border-black ${
            activeTab === "manage" ? "bg-lime-500 text-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]" : "bg-white text-gray-600 hover:bg-gray-50"
          }`}
        >
          📹 إدارة الفيديوهات
        </button>
        <button
          ...
          className={`flex-1 py-3 rounded-xl font-bold transition-all border-2 border-black relative ${
            activeTab === "review" ? "bg-lime-500 text-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]" : "bg-white text-gray-600 hover:bg-gray-50"
          }`}
        >
          ✍️ مراجعة التلخيصات
      ...
      {activeTab === "manage" ? (
        <div className="space-y-4">
          <div className="flex justify-center">
            <button
              ...
              className="bg-black text-white px-8 py-3 rounded-full font-bold hover:scale-105 transition-transform border-2 border-white"
            >
              ➕ فيديو جديد
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {items.map((item) => (
              <div key={item.id} className="bg-white rounded-2xl p-4 border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] overflow-hidden">
                ...
                <div className="flex justify-between items-center gap-2 pt-3 border-t">
                  <button
                    ...
                    className={`px-4 py-1.5 rounded-lg text-sm font-bold border-2 border-black ${
                  ...
                  <button
                    ...
                    className="bg-red-50 text-red-600 px-4 py-1.5 rounded-lg text-sm font-bold border-2 border-red-200 hover:bg-red-100"
                  >
                    حذف
                  </button>
                </div>
        ...
        <div className="space-y-4">
          {submissions.length === 0 ? (
            ...
            submissions.map((sub) => (
              <div key={sub.id} className={`bg-white rounded-2xl p-6 border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] relative overflow-hidden ${sub.status !== "pending" ? "opacity-90" : ""}`}>
                {sub.status !== "pending" && (
                ...
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="flex flex-col items-end gap-2">
                    <div className={`px-3 py-1 rounded-full text-xs font-bold border-2 border-black ${sub.is_correct ? "bg-green-500 text-white" : "bg-red-500 text-white"}`}>
                      السؤال: {sub.is_correct ? "صحيح" : "خطأ"}
                    </div>
                    {sub.status !== "pending" && (
                      <div className={`px-3 py-1 rounded-full text-[10px] font-bold border-2 border-black ${sub.status === "approved" ? "bg-lime-200 text-green-800" : "bg-red-100 text-red-800"}`}>
                    ...
                    <form onSubmit={submitReview} className="flex flex-wrap gap-3 items-center w-full">
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="bg-green-500 text-white px-4 py-2 rounded-lg font-bold border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] disabled:opacity-50 text-sm"
                        >
                          قبول
                        </button>
                        <button
                          ...
                          className="bg-red-500 text-white px-4 py-2 rounded-lg font-bold border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] disabled:opacity-50 text-sm"
                        >
                          رفض
                        </button>
                      </div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
          <div className="bg-white rounded-3xl p-8 w-full max-w-2xl border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] max-h-[90vh] overflow-y-auto" dir="rtl">
            ...
            <form onSubmit={addItem} className="space-y-4">
                  ...
                  <div key={idx} className="flex flex-col">
                    ...
                    <div className="flex gap-2 items-center">
                      ...
                      <input
                        ...
                        className="flex-1 px-4 py-2 border-2 border-black rounded-xl"
                        required
                      />
                    </div>
                  </div>
              ...
              <div className="flex gap-4 pt-6">
                <button
                  type="submit"
                  disabled={loading}
                  className="flex-1 bg-lime-500 text-black py-4 rounded-xl font-bold border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:-translate-y-1 transition-transform"
                >
                ...
                <button
                  ...
                  className="flex-1 bg-gray-200 text-black py-4 rounded-xl font-bold border-2 border-black hover:bg-gray-300 transition-colors"
                >
                  إلغاء
                </button>
              </div>
