...
function ViewerPage({ token }) {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center text-xl bg-gradient-to-br from-lime-50 to-green-50">⏳ جاري التحميل...</div>;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-lime-50 via-green-50 to-emerald-50 pb-8" dir="rtl">
      {/* Header */}
      <div className="bg-gradient-to-r from-lime-500 to-green-600 text-black py-5 shadow-lg border-b-4 border-black">
        <div className="container mx-auto px-4 text-center">
          <h1 className="text-2xl font-bold text-black">🌟 بارع</h1>
          <p className="text-green-900 mt-1">👋 مرحباً {viewerName} - وضع المشاهدة</p>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-5 space-y-5 max-w-2xl">
          ...
          <div className="divide-y divide-gray-100">
            {top10.map((s, i) => (
              <div key={s.id} className={`flex items-center gap-3 p-3 hover:bg-gray-50 transition ${i < 3 ? "bg-yellow-50/50" : ""}`}>
                ...
                <Link to={`/public/${s.id}`} className="font-semibold text-gray-800 hover:text-lime-600 text-sm flex-1">{s.name}</Link>
                <span className="bg-gradient-to-r from-lime-100 to-green-100 text-green-700 px-3 py-1 rounded-full text-sm font-bold">{s.points} ⭐</span>
              </div>
            ))}
          </div>
        </div>
                ...
                <tbody>
                  {standings.map((t, i) => (
                    <tr key={t.team} className={i === 0 ? "bg-yellow-50 font-bold" : i % 2 === 0 ? "bg-gray-50" : ""}>
                      ...
                      <td className="p-2 text-center text-lime-600">{t.won}</td>
            ...
            <div className="divide-y divide-gray-100">
              {playedMatches.map(m => (
                <div key={m.id} className="flex items-center justify-center gap-3 p-3">
                  <span className={`font-bold text-sm flex-1 text-left ${m.score1 > m.score2 ? "text-lime-600" : m.score1 < m.score2 ? "text-red-500" : "text-gray-700"}`}>{m.team1}</span>
                  <span className="bg-black text-lime-400 px-3 py-1 rounded-lg font-bold text-sm min-w-[60px] text-center border-2 border-lime-400">{m.score1} - {m.score2}</span>
                  <span className={`font-bold text-sm flex-1 text-right ${m.score2 > m.score1 ? "text-lime-600" : m.score2 < m.score1 ? "text-red-500" : "text-gray-700"}`}>{m.team2}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {supervisors.map(sup => {
          ...
          return (
            <div key={sup} className="bg-white rounded-2xl shadow-xl overflow-hidden border border-green-100">
              <div className="bg-gradient-to-r from-green-500 to-emerald-500 text-black p-3 flex items-center justify-between border-b-2 border-black">
                <h3 className="font-bold text-sm">🏅 {sup}</h3>
                <span className="bg-black text-white px-2 py-0.5 rounded-full text-xs">{groupStudents.length} طالب</span>
              </div>
              <div className="divide-y divide-gray-100">
                {groupStudents.map((s, i) => (
                  <div key={s.id} className="flex items-center gap-3 p-3 hover:bg-gray-50 transition">
                    ...
                    <Link to={`/public/${s.id}`} className="font-semibold text-gray-800 hover:text-lime-600 text-sm flex-1">{s.name}</Link>
                    <span className="bg-lime-100 text-green-700 px-2 py-0.5 rounded-full text-xs font-bold">{s.points} ⭐</span>
                  </div>
                ))}
              </div>
            </div>
