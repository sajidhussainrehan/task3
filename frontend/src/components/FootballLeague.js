  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        <button onClick={() => setActiveTab("standings")} className={`flex-1 py-2 rounded-lg font-bold text-sm border-2 ${activeTab === "standings" ? "bg-lime-500 text-black border-black" : "bg-white text-gray-600 shadow border-gray-200"}`}>🏆 جدول الترتيب</button>
      ...
      <button onClick={() => setShowAddMatch(true)} className="w-full bg-lime-500 hover:bg-lime-600 text-black py-2 rounded-lg font-bold text-sm border-2 border-black" data-testid="add-match-btn">
        ⚽ جدولة مباراة جديدة
      </button>
                ...
                <tbody>
                  {standings.map((t, i) => (
                    <tr key={t.team} className={i === 0 ? "bg-yellow-50 font-bold" : i % 2 === 0 ? "bg-gray-50" : ""}>
                      ...
                      <td className="px-2 py-3 text-center font-bold text-lime-600">{t.wins || 0}</td>
                      ...
                      <td className="p-2 text-center font-bold text-lime-600 text-lg">{t.points}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {pendingMatches.length > 0 ? pendingMatches.map(m => (
            <div key={m.id} className="bg-white rounded-xl shadow p-4 border-r-4 border-blue-500">
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <button onClick={() => openScoreModal(m)} className="bg-lime-500 hover:bg-lime-600 text-black px-3 py-1 rounded-lg text-xs font-bold border border-black" data-testid={`score-btn-${m.id}`}>
                    إدخال النتيجة
                  </button>
      ...
      {activeTab === "played" && (
        <div className="space-y-3">
          {playedMatches.length > 0 ? playedMatches.map(m => (
            <div key={m.id} className="bg-white rounded-xl shadow p-4 border-r-4 border-lime-500">
              <div className="flex items-center justify-between">
                <div className="flex-1 flex items-center justify-center gap-3">
                  <span className={`font-bold text-sm flex-1 text-left ${m.score1 > m.score2 ? "text-lime-600" : m.score1 < m.score2 ? "text-red-600" : "text-gray-800"}`}>{m.team1}</span>
                  <span className="bg-black text-lime-400 px-3 py-1 rounded-lg font-bold min-w-[70px] text-center text-sm border-2 border-lime-400">
                    {m.score1} - {m.score2}
                  </span>
                  <span className={`font-bold text-sm flex-1 text-right ${m.score2 > m.score1 ? "text-lime-600" : m.score2 < m.score1 ? "text-red-600" : "text-gray-800"}`}>{m.team2}</span>
                </div>
                <div className="flex gap-2">
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-md" onClick={e => e.stopPropagation()} dir="rtl">
            ...
            <form onSubmit={addMatch} className="space-y-4">
              <div>
                ...
                <select value={team1} onChange={e => setTeam1(e.target.value)} className="w-full px-4 py-3 border-2 rounded-lg focus:outline-none focus:border-lime-500" required data-testid="match-team1">
              ...
              <div>
                ...
                <select value={team2} onChange={e => setTeam2(e.target.value)} className="w-full px-4 py-3 border-2 rounded-lg focus:outline-none focus:border-lime-500" required data-testid="match-team2">
                  <option value="">اختر الفريق</option>
                  {supervisors.map(s => <option key={s} value={s}>{s}</option>)}
                </select>
              </div>
              <div className="flex gap-3">
                <button type="submit" disabled={loading} className="flex-1 bg-lime-500 hover:bg-lime-600 text-black py-3 rounded-lg font-bold disabled:opacity-50 border-2 border-black" data-testid="submit-match">
            ...
            <form onSubmit={updateScore} className="space-y-4">
              <div className="flex gap-4 items-center">
                <div className="flex-1 text-center">
                  ...
                  <input type="number" min="0" value={score1} onChange={e => setScore1(parseInt(e.target.value) || 0)} className="w-full px-3 py-3 border-2 rounded-lg text-center text-2xl font-bold focus:outline-none focus:border-lime-500" data-testid="score-input-1" />
                </div>
                ...
                <div className="flex-1 text-center">
                  ...
                  <input type="number" min="0" value={score2} onChange={e => setScore2(parseInt(e.target.value) || 0)} className="w-full px-3 py-3 border-2 rounded-lg text-center text-2xl font-bold focus:outline-none focus:border-lime-500" data-testid="score-input-2" />
                </div>
              </div>
              <div className="flex gap-3">
                <button type="submit" disabled={loading} className="flex-1 bg-lime-500 hover:bg-lime-600 text-black py-3 rounded-lg font-bold disabled:opacity-50 border-2 border-black" data-testid="submit-score">
