  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-lime-100">
      ...
      <div className="p-4 space-y-4">
        {message && (
          <div className="bg-lime-50 border-r-4 border-lime-500 p-3 rounded text-sm text-green-700 font-bold mb-4 animate-fadeIn">
            {message}
          </div>
        )}
        ...
        {items.map((item) => {
          ...
          return (
            <div key={item.id} className={`border-2 rounded-2xl overflow-hidden transition-all duration-300 ${isExpanded ? "border-black shadow-lg" : "border-gray-100"}`}>
              {/* Card Header/Preview */}
              <div 
                className={`p-4 cursor-pointer flex items-center justify-between ${isExpanded ? "bg-lime-50" : "bg-white hover:bg-gray-50"}`}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    {submission ? (
                      ...
                      <p className="text-xs text-gray-500">اضغط للمشاهدة والحل - <span className="text-lime-600 font-bold">💎 {item.points_question + item.points_summary} نقطة محتملة</span></p>
                    )}
                  </div>
                </div>
                {!submission && (
              ...
              {isExpanded && !submission && (
                <div className="p-4 space-y-4 bg-white border-t-2 border-dashed border-gray-200">
                  {/* YouTube Embed */}
                  <div className="aspect-video rounded-xl overflow-hidden bg-black shadow-inner border shadow-lg border-black">
                    <iframe
                      width="100%"
                      height="100%"
                      ...
                      {item.options.map((opt, idx) => (
                        <button
                          ...
                          className={`p-3 rounded-xl border-2 text-right transition-all font-bold text-sm ${
                            formData.answer === idx 
                            ? "border-black bg-lime-400 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]" 
                  ...
                  <div className="space-y-2 pt-4 border-t border-gray-100">
                    ...
                    <textarea
                      ...
                      className="w-full p-4 border-2 border-gray-200 rounded-xl focus:border-black outline-none min-h-[100px] text-sm"
                  ...
                  <button
                    ...
                    className="w-full bg-black text-white py-4 rounded-xl font-bold shadow-[4px_4px_0px_0px_rgba(101,163,13,1)] disabled:opacity-50 disabled:shadow-none transition-all active:translate-y-1 active:shadow-none"
                  >
