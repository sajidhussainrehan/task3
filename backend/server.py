...
class Team(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    name: str  # Usually matches student group/supervisor name
    group_photo: Optional[str] = None
    lineup: List[dict] = []  # List of {player_name, position_x, position_y}
    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
...
async def teacher_login(data: TeacherLoginRequest):
    ...
    teacher = await db.teachers.find_one({"username": data.username})
...
async def get_challenges():
    ...
    for c in challenges:
        ...
        # Migration: handle old is_active field
        if "active" not in c and "is_active" in c:
...
async def get_active_challenges():
    now = datetime.now(timezone.utc).isoformat()
    # Support both 'active' and 'is_active' for legacy data
...
async def create_challenge(data: ChallengeCreate):
    ...
    doc["active"] = True  # Default to active
    doc["created_at"] = doc["created_at"].isoformat()
